...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		/* 취소 */
		$('#btnStm2102Cancle').click(function() {
			gfnLayerPopupClose();
		});
	});
...
function fnAuthRefresh(){
	...
	var ajaxObj = new gfnAjaxRequestAction(
			{"url":"<c:url value='/stm/stm2000/stm2000/selectStm2000SvnAuthGrpList.do'/>","loadingShow":false},
			{prjId:'${param.prjId}',svnRepId: "${param.svnRepId}"});
	//AJAX 전송 성공 함수
	ajaxObj.setFnSuccess(function(data){
		...
		//에러 없는경우
		if(data.errorYN != "Y"){
			authGrid.setData(data.svnAuthGrpList);
		}else{
...
function fnAuthInsert(){
	...
	gfnCommonAuthPopup("${param.prjId}", "", true,function(objs){
		if(objs.length>0){
			...
			selAuthFd.append("svnRepId","${param.svnRepId}");
			...
			//AJAX 설정
			var ajaxObj = new gfnAjaxRequestAction(
					{"url":"<c:url value='/stm/stm2000/stm2000/insertStm2000SvnAuthGrpList.do'/>"
						,"contentType":false
						,"processData":false
						,"cache":false},
					selAuthFd)
...
function fnAuthDelete(){
	...
	//AJAX 설정
	var ajaxObj = new gfnAjaxRequestAction(
			{"url":"<c:url value='/stm/stm2000/stm2000/deleteStm2000SvnAuthGrpList.do'/>"
				,"contentType":false
				,"processData":false
				,"cache":false},
			selAuthFd)
...
<div class="popup">
	<div class="pop_title">SVN Repository 허용 역할그룹 설정</div>
	<div class="pop_sub">
		...
		<div class="btn_div">
			<div class="button_normal exit_btn" id="btnStm2102Cancle" >닫기</div>
		</div>
